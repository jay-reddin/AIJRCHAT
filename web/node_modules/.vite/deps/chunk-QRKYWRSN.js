...
var promises = __toESM(require_promises(), 1);
...
var SHARED_RECURSION_TRACKER = new EntityPathTracker();
...
var UNKNOWN_INCLUDED_PATH = Object.freeze({ [UnknownKey]: EMPTY_OBJECT });
var IncludedFullPathTracker = class {
  constructor() {
  ...
  includePathAndGetIfIncluded(path) {
    ...
    let currentPaths = this.includedPaths || (this.includedPaths = (included = false, /* @__PURE__ */ Object.create(null)));
    for (const pathSegment of path) {
      ...
      currentPaths = currentPaths[pathSegment] || (currentPaths[pathSegment] = (included = false, /* @__PURE__ */ Object.create(null)));
    }
    return included;
  }
};
var UNKNOWN_INCLUDED_TOP_LEVEL_PATH = Object.freeze({
  [UnknownKey]: true
});
var IncludedTopLevelPathTracker = class {
  constructor() {
    this.includedPaths = null;
  }
  includePathAndGetIfIncluded(path) {
    let included = true;
    const includedPaths = this.includedPaths || (this.includedPaths = (included = false, /* @__PURE__ */ Object.create(null)));
...
var UnknownFalsyValue = Symbol("Unknown Falsy Value");
...
var includeInteraction = (interaction, context) => {
  var _a;
  (_a = interaction.args[0]) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context);
  includeInteractionWithoutThis(interaction, context);
};
var includeInteractionWithoutThis = ({ args }, context) => {
...
var Variable = class extends ExpressionEntity {
  ...
  includePath(path, context) {
    var _a;
    this.included = true;
    (_a = this.renderedLikeHoisted) == null ? void 0 : _a.includePath(path, context);
  }
  /**
...
var IS_SKIPPED_CHAIN = Symbol("IS_SKIPPED_CHAIN");
var NodeBase = class extends ExpressionEntity {
  ...
  includeNode(context) {
    ...
    for (const key of childNodeKeys[this.type]) {
      ...
      if (Array.isArray(value)) {
        for (const child of value) {
          child == null ? void 0 : child.includePath(UNKNOWN_PATH, context);
        }
      } else {
...
var UNDEFINED_EXPRESSION = new class UndefinedExpression extends ExpressionEntity {
  getLiteralValueAtPath(path) {
    return path.length > 0 ? UnknownValue : void 0;
  }
}();
var returnsUnknown = {
  value: {
...
var ObjectEntity = class extends ExpressionEntity {
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker) {
    ...
    (type === INTERACTION_CALLED || path.length > 1) && (this.hasUnknownDeoptimizedProperty || typeof key === "string" && this.deoptimizedPaths[key])) {
      deoptimizeInteraction(interaction);
      return;
    }
    ...
    (_a = this.prototypeExpression) == null ? void 0 : _a.deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker);
  }
  ...
  deoptimizePath(path) {
    ...
    (_a = this.prototypeExpression) == null ? void 0 : _a.deoptimizePath(path.length === 1 ? [path[0], UnknownKey] : path);
  }
  ...
  include(context, includeChildrenRecursively) {
    var _a;
    this.included = true;
    for (const property2 of this.allProperties) {
      if (includeChildrenRecursively || property2.shouldBeIncluded(context)) {
        property2.include(context, includeChildrenRecursively);
      }
    }
    (_a = this.prototypeExpression) == null ? void 0 : _a.include(context, includeChildrenRecursively);
  }
  includePath(path, context) {
    var _a;
    this.included = true;
    ...
    for (const property2 of includedMembers) {
      property2.includePath(includedPath, context);
    }
    (_a = this.prototypeExpression) == null ? void 0 : _a.includePath(path, context);
  }
...
var OBJECT_PROTOTYPE_FALLBACK = new class ObjectPrototypeFallbackExpression extends ExpressionEntity {
  ...
  getLiteralValueAtPath(path) {
    return path.length === 1 && isInteger(path[0]) ? void 0 : UnknownValue;
  }
  hasEffectsOnInteractionAtPath(path, { type }) {
...
var ArrayExpression = class extends NodeBase {
  ...
  includeNode(context) {
    ...
    for (const element of this.elements) {
      if (element) {
        element == null ? void 0 : element.includePath(UNKNOWN_PATH, context);
      }
    }
  }
  applyDeoptimizations() {
...
var getUnknownValue = () => UnknownValue;
    ...
    [ValueProperties]: {
      deoptimizeArgumentsOnCall: ({ args: [, target, parameter] }) => {
        if (isObjectExpressionNode(parameter)) {
          const hasSpreadElement = parameter.properties.some((property2) => !isPropertyNode(property2));
          if (!hasSpreadElement) {
            for (const property2 of parameter.properties) {
              property2.deoptimizeArgumentsOnInteractionAtPath({
                args: [null, target],
                type: INTERACTION_CALLED,
                withNew: false
...
var limitConcatenatedPathDepth = (path1, path2) => {
  const { length: length1 } = path1;
  const { length: length2 } = path2;
  return length1 === 0 ? path2 : length2 === 0 ? path1 : length1 + length2 > MAX_PATH_DEPTH ? [...path1, ...path2.slice(0, MAX_PATH_DEPTH - 1 - path1.length), "UnknownKey"] : [...path1, ...path2];
};
var LocalVariable = class extends Variable {
  ...
  addDeclaration(identifier2, init) {
    ...
    switch (interaction.type) {
      case INTERACTION_ACCESSED: {
        if (this.isReassigned)
          return true;
        return !context.accessed.trackEntityAtPathAndGetIfTracked(path, this) && this.init.hasEffectsOnInteractionAtPath([...this.initPath, ...path], interaction, context);
      }
      case INTERACTION_ASSIGNED: {
        if (this.included)
          return true;
        ...
        if (this.isReassigned)
          return true;
        return !context.assigned.trackEntityAtPathAndGetIfTracked(path, this) && this.init.hasEffectsOnInteractionAtPath([...this.initPath, ...path], interaction, context);
      }
      case INTERACTION_CALLED: {
        if (this.isReassigned)
          return true;
        return !(interaction.withNew ? context.instantiated : context.called).trackEntityAtPathAndGetIfTracked(path, interaction.args, this) && this.init.hasEffectsOnInteractionAtPath([...this.initPath, ...path], interaction, context);
      }
    }
  }
  includePath(path, context) {
    ...
    if (!this.includedPathTracker.includePathAndGetIfIncluded(path)) {
      ...
      if (path.length > 0) {
        ...
        (_a = this.additionalInitializers) == null ? void 0 : _a.forEach((initializer) => initializer.includePath(UNKNOWN_PATH, context));
      }
    }
  }
  includeCallArguments(interaction, context) {
    if (this.isReassigned || context.includedCallArguments.has(this.init) || // This can be removed again once we can include arguments when called at
    // a specific path
...
var IdentifierBase = class extends NodeBase {
  constructor() {
    super(...arguments);
    this.variable = null;
  ...
  deoptimizePath(path) {
    ...
    (_a = this.variable) == null ? void 0 : _a.deoptimizePath(path);
  }
  getLiteralValueAtPath(path, recursionTracker, origin) {
  ...
  include(context, includeChildrenRecursively) {
    ...
    if (includeChildrenRecursively) {
      (_a = this.variable) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context);
    }
  }
  includeNode(context) {
    this.included = true;
  ...
  includePath(path, context) {
    ...
    if (!this.included) {
      ...
      (_a = this.variable) == null ? void 0 : _a.includePath(path, context);
    }
  }
  includeCallArguments(interaction, context) {
...
var Identifier2 = class extends IdentifierBase {
  constructor() {
    super(...arguments);
    this.variable = null;
  }
  ...
  hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
    return destructuredInitPath.length > 0 && init.hasEffectsOnInteractionAtPath(destructuredInitPath, NODE_INTERACTION_UNKNOWN_ACCESS, context);
  }
  includeDestructuredIfNecessary(context, destructuredInitPath, init) {
    ...
    const { propertyReadSideEffects } = this.scope.context.options.treeshake;
    if (this.included || (this.included = destructuredInitPath.length > 0 && !context.brokenFlow && propertyReadSideEffects && (propertyReadSideEffects === "always" || init.hasEffectsOnInteractionAtPath(destructuredInitPath, NODE_INTERACTION_UNKNOWN_ACCESS, createHasEffectsContext())))) {
...
var Scope = class {
   ...
   */
  addDeclaration(identifier2, context, init, destructuredInitPath, kind) {
    var _a;
    ...
    if (existingVariable) {
      if (kind === "var" && existingVariable.kind === "var") {
        existingVariable.addDeclaration(identifier2, init);
        return existingVariable;
      }
      context.error(logRedeclarationError(name), identifier2.start);
    }
    const newVariable = new LocalVariable(identifier2.name, identifier2, init, destructuredInitPath, context, kind);
...
var BlockScope = class extends ChildScope {
  ...
  addDeclaration(identifier2, context, init, destructuredInitPath, kind) {
    ...
    if (kind === "var") {
      ...
      const declaredVariable = this.parent.addDeclaration(identifier2, context, init, destructuredInitPath, kind);
      declaredVariable.markInitializersForDeoptimization();
      this.addHoistedVariable(name, declaredVariable);
      return declaredVariable;
    }
    return super.addDeclaration(identifier2, context, init, destructuredInitPath, kind);
  }
};
var StaticBlock2 = class extends NodeBase {
  createScope(parentScope) {
...
var ClassNode = class extends NodeBase {
  ...
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    var _a;
    return interaction.type === INTERACTION_CALLED && path.length === 0 ? !interaction.withNew || (this.classConstructor === null ? (_a = this.superClass) == null ? void 0 : _a.hasEffectsOnInteractionAtPath(path, interaction, context) : this.classConstructor.hasEffectsOnInteractionAtPath(path, interaction, context)) || false : this.getObjectEntity().hasEffectsOnInteractionAtPath(path, interaction, context);
  }
  include(context, includeChildrenRecursively) {
    var _a;
    if (!this.included)
    ...
    for (const decorator2 of this.decorators)
      decorator2.include(context, includeChildrenRecursively);
    if (this.id) {
  ...
  initialise() {
    var _a;
    super.initialise();
    (_a = this.id) == null ? void 0 : _a.declare("class", EMPTY_PATH, this);
  ...
  render(code, options) {
    ...
    if (this.id) {
      ...
      if (renderedVariable !== name) {
        this.decorators.map((decorator2) => decorator2.render(code, options));
...
var ParameterVariable = class extends LocalVariable {
  ...
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    ...
    if (this.isReassigned || type === INTERACTION_ASSIGNED || path.length + this.initPath.length > MAX_PATH_DEPTH) {
      return super.hasEffectsOnInteractionAtPath(path, interaction, context);
    }
    return !(type === INTERACTION_CALLED ? (interaction.withNew ? context.instantiated : context.called).trackEntityAtPathAndGetIfTracked(path, interaction.args, this) : context.accessed.trackEntityAtPathAndGetIfTracked(path, this)) && this.getKnownValue().hasEffectsOnInteractionAtPath([...this.initPath, ...path], interaction, context);
  }
  deoptimizeArgumentsOnInteractionAtPath(interaction, path) {
    if (path.length >= 2 || this.argumentsToBeDeoptimized.has(UNKNOWN_EXPRESSION) || this.deoptimizationInteractions.length >= MAX_TRACKED_INTERACTIONS || path.length === 1 && (this.deoptimizedFields.has(UnknownKey) || interaction.type === INTERACTION_CALLED && this.deoptimizedFields.has(path[0])) || this.initPath.length + path.length > MAX_PATH_DEPTH) {
      deoptimizeInteraction(interaction);
      return;
    }
    ...
    if (kind === "var") {
      ...
      if (existingVariable) {
        ...
        existingVariable.declarations[0].parent.type === CatchClause) {
          const declaredVariable2 = this.parent.parent.addDeclaration(identifier2, context, UNDEFINED_EXPRESSION, destructuredInitPath, kind);
      ...
      const declaredVariable = this.parent.parent.addDeclaration(identifier2, context, init, destructuredInitPath, kind);
      declaredVariable.markInitializersForDeoptimization();
      this.addHoistedVariable(name, declaredVariable);
      return declaredVariable;
    }
...
var ParameterScope = class extends ChildScope {
   ...
   */
  addParameterDeclaration(identifier2, argumentPath) {
    const { name, start } = identifier2;
    const existingParameter = this.variables.get(name);
    if (existingParameter) {
      return this.context.error(logDuplicateArgumentNameError(name), start);
    }
    const variable = new ParameterVariable(name, identifier2, argumentPath, this.context);
    ...
    for (; position < args.length - 1; position++) {
      ...
      if (argument instanceof SpreadElement) {
        for (; position < parameters.length; position++) {
          (_a = args[position + 1]) == null ? void 0 : _a.deoptimizePath(UNKNOWN_PATH);
...
var RestElement2 = class extends NodeBase {
  constructor() {
    super(...arguments);
  ...
  includeDestructuredIfNecessary(context, destructuredInitPath, init) {
    return this.included = this.argument.includeDestructuredIfNecessary(context, getIncludedPatternPath$1(destructuredInitPath), init) || this.included;
  }
  include(context, includeChildrenRecursively) {
    if (!this.included)
      this.includeNode(context);
...
var getIncludedPatternPath$1 = (destructuredInitPath) => destructuredInitPath.at(-1) === UnknownKey ? destructuredInitPath : [...destructuredInitPath, UnknownKey];
var FunctionBase = class extends NodeBase {
  ...
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    ...
    if (this.async) {
      ...
      if (returnExpression.hasEffectsOnInteractionAtPath(["then"], NODE_INTERACTION_UNKNOWN_CALL, context) || propertyReadSideEffects2 && (propertyReadSideEffects2 === "always" || returnExpression.hasEffectsOnInteractionAtPath(["then"], NODE_INTERACTION_UNKNOWN_ACCESS, context))) {
    ...
    for (let index = 0; index < this.params.length; index++) {
      ...
      if (parameter.hasEffects(context) || propertyReadSideEffects && parameter.hasEffectsWhenDestructuring(context, EMPTY_PATH, interaction.args[index + 1] || UNDEFINED_EXPRESSION)) {
        this.hasCachedEffects = true;
        return true;
      }
    }
  ...
  parseNode(esTreeNode) {
    ...
    scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement2);
...
var FunctionNode = class extends FunctionBase {
  ...
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    if (this.annotationNoSideEffects && path.length === 0 && interaction.type === INTERACTION_CALLED) {
      return false;
    }
  ...
  include(context, includeChildrenRecursively) {
    ...
    (_a = this.id) == null ? void 0 : _a.include(context, includeChildrenRecursively);
  ...
  initialise() {
    var _a;
    super.initialise();
    (_a = this.id) == null ? void 0 : _a.declare("function", EMPTY_PATH, this);
  }
  getObjectEntity() {
    if (this.objectEntity !== null) {
      return this.objectEntity;
...
var HELPER_GENERATORS = {
  [DOCUMENT_CURRENT_SCRIPT](_t, { _, n: n2 }) {
    return `var ${DOCUMENT_CURRENT_SCRIPT}${_}=${_}typeof document${_}!==${_}'undefined'${_}?${_}document.currentScript${_}:${_}null;${n2}`;
  },
...
var MemberExpression2 = class extends NodeBase {
  constructor() {
    super(...arguments);
    this.variable = null;
    this.expressionsToBeDeoptimized = [];
  ...
  deoptimizePath(path) {
    ...
    if (this.variable) {
      ...
      this.object.deoptimizePath([
        ...
        ...path.length < MAX_PATH_DEPTH ? path : [...path.slice(0, MAX_PATH_DEPTH), UnknownKey]
      ]);
    }
  }
  getLiteralValueAtPath(path, recursionTracker, origin) {
  ...
  hasEffectsAsChainElement(context) {
    ...
    const objectHasEffects = "hasEffectsAsChainElement" in this.object ? this.object.hasEffectsAsChainElement(context) : this.object.hasEffects(context);
  ...
  includeDestructuredIfNecessary(context, destructuredInitPath, init) {
    if (this.included || (this.included = destructuredInitPath.length > 0 && !context.brokenFlow && init.hasEffectsOnInteractionAtPath(destructuredInitPath, NODE_INTERACTION_UNKNOWN_ACCESS, createHasEffectsContext()))) {
  ...
  hasAccessEffect(context) {
    const { propertyReadSideEffects } = this.scope.context.options.treeshake;
    return !(this.variable || this.isUndefined) && propertyReadSideEffects && (propertyReadSideEffects === "always" || this.object.hasEffectsOnInteractionAtPath([this.getDynamicPropertyKey()], this.accessInteraction, context));
  }
};
function resolveNamespaceVariables(baseVariable, path, astContext) {
  ...
  if (!variable) {
    if (path.length === 1) {
      ...
      astContext.log(LOGLEVEL_WARN, logMissingExport(exportName, astContext.module.id, fileName, !!(options == null ? void 0 : options.missingButExportExists)), path[0].pos);
      return "undefined";
    }
    return null;
  }
...
var MetaProperty = class extends NodeBase {
  ...
  includeNode() {
    ...
    if (this.meta.name === IMPORT) {
      ...
      const metaProperty2 = this.metaProperty = parent instanceof MemberExpression2 && typeof parent.propertyKey === "string" ? parent.propertyKey : null;
...
var getResolveUrl = (path, URL2 = "URL") => `new ${URL2}(${path}).href`;
var getRelativeUrlFromDocument = (relativePath, umd2 = false) => getResolveUrl(`'${escapeId(relativePath)}', ${umd2 ? `typeof document === 'undefined' ? location.href : ` : ""}document.currentScript && document.currentScript.tagName.toUpperCase() === 'SCRIPT' && document.currentScript.src || document.baseURI`);
...
var getUrlFromDocument = (chunkId, umd2 = false) => `${umd2 ? `typeof document === 'undefined' ? location.href : ` : ""}(${DOCUMENT_CURRENT_SCRIPT} && ${DOCUMENT_CURRENT_SCRIPT}.tagName.toUpperCase() === 'SCRIPT' && ${DOCUMENT_CURRENT_SCRIPT}.src || new URL('${escapeId(chunkId)}', document.baseURI).href)`;
...
var ExportDefaultVariable = class _ExportDefaultVariable extends LocalVariable {
  constructor(name, exportDefaultDeclaration2, context) {
    super(name, exportDefaultDeclaration2, exportDefaultDeclaration2.declaration, EMPTY_PATH, context, "other");
    this.hasId = false;
...
var NamespaceVariable = class extends Variable {
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker) {
    ...
    if (path.length > 1 || path.length === 1 && interaction.type === INTERACTION_CALLED) {
      const key = path[0];
      if (typeof key === "string") {
        (_a = this.getMemberVariables()[key]) == null ? void 0 : _a.deoptimizeArgumentsOnInteractionAtPath(interaction, path.slice(1), recursionTracker);
      } else {
        deoptimizeInteraction(interaction);
      }
    }
  ...
  deoptimizePath(path) {
    ...
    if (path.length > 1) {
      const key = path[0];
      if (typeof key === "string") {
        (_a = this.getMemberVariables()[key]) == null ? void 0 : _a.deoptimizePath(path.slice(1));
      }
    }
  }
  getLiteralValueAtPath(path) {
...
var ExternalChunk = class {
  ...
  getImportAttributes(snippets) {
    return this.importAttributes || (this.importAttributes = formatAttributes(["es", "cjs"].includes(this.options.format) && this.options.externalImportAttributes && this.moduleInfo.attributes, snippets));
  }
...
function amd(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, id, indent: t, intro, isEntryFacade, isModuleFacade, namedExportsMode, log, outro, snippets }, { amd: amd2, esModule, externalLiveBindings, freeze, generatedCode: { symbols }, interop, reexportProtoFromExternal, strict }) {
  ...
  if (hasExports && (namedExportsMode || ((_a = exports[0]) == null ? void 0 : _a.local) === "exports.default")) {
...
function umd(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, id, indent: t, intro, namedExportsMode, log, outro, snippets }, { amd: amd2, compact, esModule, extend, externalLiveBindings, freeze, interop, name, generatedCode: { symbols }, globals, noConflict, reexportProtoFromExternal, strict }) {
  ...
  if ((hasExports || noConflict) && (namedExportsMode || hasExports && ((_a = exports[0]) == null ? void 0 : _a.local) === "exports.default")) {
...
var picomatch = getDefaultExportFromCjs(picomatchExports);
var extractors = {
...
var createFilter$1 = function createFilter(include, exclude, options) {
...
var ArrayPattern = class extends NodeBase {
  ...
  deoptimizeAssignment(destructuredInitPath, init) {
    const includedPatternPath = getIncludedPatternPath(destructuredInitPath);
    for (const element of this.elements) {
      element == null ? void 0 : element.deoptimizeAssignment(includedPatternPath, init);
    }
  }
  ...
  hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
    const includedPatternPath = getIncludedPatternPath(destructuredInitPath);
    for (const element of this.elements) {
      if (element == null ? void 0 : element.hasEffectsWhenDestructuring(context, includedPatternPath, init)) {
        return true;
      }
    }
    return false;
  ...
  includeDestructuredIfNecessary(context, destructuredInitPath, init) {
    ...
    for (const element of this.elements) {
      if (element) {
        element.included || (element.included = included);
        included = element.includeDestructuredIfNecessary(context, includedPatternPath, init) || included;
      }
    }
    if (included) {
      for (const element of this.elements) {
    ...
    return this.included || (this.included = included);
  }
  markDeclarationReached() {
    for (const element of this.elements) {
...
var getIncludedPatternPath = (destructuredInitPath) => destructuredInitPath.at(-1) === UnknownKey ? destructuredInitPath : [...destructuredInitPath, UnknownInteger];
...
var ObjectPattern = class extends NodeBase {
  ...
  declare(kind, destructuredInitPath, init) {
    const variables = [];
    for (const property2 of this.properties) {
      variables.push(...property2.declare(kind, destructuredInitPath, init));
    }
    return variables;
  }
  deoptimizeAssignment(destructuredInitPath, init) {
    for (const property2 of this.properties) {
      property2.deoptimizeAssignment(destructuredInitPath, init);
    }
  }
  deoptimizePath(path) {
    if (path.length === 0) {
  ...
  hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
    for (const property2 of this.properties) {
      if (property2.hasEffectsWhenDestructuring(context, destructuredInitPath, init))
        return true;
    }
    return false;
  }
  includeDestructuredIfNecessary(context, destructuredInitPath, init) {
    ...
    for (const property2 of this.properties.slice(0, -1)) {
      if (lastPropertyIsRestElement && lastPropertyIncluded) {
        property2.includeNode(context);
      }
      included = property2.includeDestructuredIfNecessary(context, destructuredInitPath, init) || included;
    }
    return this.included || (this.included = included);
  }
  markDeclarationReached() {
...
var AssignmentExpression = class extends NodeBase {
  ...
  hasEffects(context) {
    ...
    if (!deoptimized)
      this.applyDeoptimizations();
    return isConstReassignment || right.hasEffects(context) || left.hasEffectsAsAssignmentTarget(context, operator !== "=") || ((_b = (_a = this.left).hasEffectsWhenDestructuring) == null ? void 0 : _b.call(_a, context, EMPTY_PATH, right));
  }
  hasEffectsOnInteractionAtPath(path, interaction, context) {
  ...
  include(context, includeChildrenRecursively) {
    ...
    if (includeChildrenRecursively || isConstReassignment || operator !== "=" || left.included || left.hasEffectsAsAssignmentTarget(hasEffectsContext, false) || ((_a = left.hasEffectsWhenDestructuring) == null ? void 0 : _a.call(left, hasEffectsContext, EMPTY_PATH, right))) {
...
var AssignmentPattern = class extends NodeBase {
  ...
  includeDestructuredIfNecessary(context, destructuredInitPath, init) {
    let included = this.left.includeDestructuredIfNecessary(context, destructuredInitPath, init) || this.included;
    if (included || (included = this.right.shouldBeIncluded(context))) {
...
var AwaitExpression2 = class extends NodeBase {
  hasEffects() {
    if (!this.deoptimized)
      this.applyDeoptimizations();
    return true;
...
function getSimplifiedNumber(value) {
  ...
  const floatLength = ((_a = base2.split(".")[1]) == null ? void 0 : _a.length) || 0;
...
var UNASSIGNED$1 = Symbol("Unassigned");
...
var CallExpressionBase = class extends NodeBase {
  ...
  deoptimizePath(path) {
    if (path.length === 0 || this.scope.context.deoptimizationTracker.trackEntityAtPathAndGetIfTracked(path, this)) {
      return;
    }
...
var CallExpression2 = class extends CallExpressionBase {
  ...
  hasEffectsAsChainElement(context) {
    const calleeHasEffects = "hasEffectsAsChainElement" in this.callee ? this.callee.hasEffectsAsChainElement(context) : this.callee.hasEffects(context);
    ...
    return !this.annotationPure && (calleeHasEffects || this.callee.hasEffectsOnInteractionAtPath(EMPTY_PATH, this.interaction, context));
  }
  include(context, includeChildrenRecursively) {
    if (!this.included)
      this.includeNode(context);
...
var MultiExpression = class _MultiExpression extends ExpressionEntity {
  ...
  getReturnExpressionWhenCalledAtPath(path, interaction, recursionTracker, origin) {
    return [
      new _MultiExpression(this.expressions.map((expression) => expression.getReturnExpressionWhenCalledAtPath(path, interaction, recursionTracker, origin)[0])),
      false
    ];
  }
  hasEffectsOnInteractionAtPath(path, interaction, context) {
...
var Decorator = class extends NodeBase {
  hasEffects(context) {
    return this.expression.hasEffects(context) || this.expression.hasEffectsOnInteractionAtPath(EMPTY_PATH, NODE_INTERACTION_UNKNOWN_CALL, context);
  }
};
    ...
    if (this.declaration === null) {
      ...
      if (this.declaration instanceof ClassDeclaration) {
        ...
        for (const decorator2 of decorators) {
          endBoundary = Math.min(endBoundary, decorator2.start);
        }
...
var ForStatement = class extends NodeBase {
  ...
  include(context, includeChildrenRecursively) {
    var _a, _b, _c;
    this.included = true;
    (_a = this.init) == null ? void 0 : _a.include(context, includeChildrenRecursively, {
...
var FunctionExpression2 = class extends FunctionNode {
  createScope(parentScope) {
...
var VariableDeclarator2 = class extends NodeBase {
  ...
  hasEffects(context) {
    ...
    this.id.markDeclarationReached();
    return initEffect || this.isUsingDeclaration || this.id.hasEffects(context) || this.scope.context.options.treeshake.propertyReadSideEffects && this.id.hasEffectsWhenDestructuring(context, EMPTY_PATH, this.init || UNDEFINED_EXPRESSION);
  }
  include(context, includeChildrenRecursively) {
    const { id, init } = this;
    if (!this.included)
      this.includeNode();
    init == null ? void 0 : init.include(context, includeChildrenRecursively);
  ...
  render(code, options) {
    ...
    if (init) {
      if (id instanceof Identifier2 && init instanceof ClassExpression && !init.id) {
  ...
  includeNode() {
    this.included = true;
    const { id, init } = this;
    if (init && id instanceof Identifier2 && init instanceof ClassExpression && !init.id) {
      const { name, variable } = id;
...
var ImportExpression2 = class extends NodeBase {
  constructor() {
    super(...arguments);
  ...
  getDeterministicImportedNames() {
    ...
    if (parent1 instanceof MemberExpression2) {
      ...
      const thenCallback = callExpression2.arguments[0];
      if (callExpression2.arguments.length !== 1 || !(thenCallback instanceof ArrowFunctionExpression2 || thenCallback instanceof FunctionExpression2)) {
        return;
      }
...
var JSXIdentifier = class extends IdentifierBase {
  constructor() {
    super(...arguments);
...
var JSXAttribute = class extends NodeBase {
...
var JSXClosingBase = class extends NodeBase {
  render(code, options) {
...
var JSXClosingElement = class extends JSXClosingBase {
};
var JSXClosingFragment = class extends JSXClosingBase {
};
var JSXSpreadAttribute = class extends NodeBase {
  render(code, options) {
...
var JSXEmptyExpression = class extends NodeBase {
};
JSXEmptyExpression.prototype.includeNode = onlyIncludeSelf;
var JSXExpressionContainer = class extends NodeBase {
  includeNode(context) {
    this.included = true;
    if (!this.deoptimized)
      this.applyDeoptimizations();
...
var JSXElementBase = class extends NodeBase {
  constructor() {
    super(...arguments);
...
var JSXElement = class extends JSXElementBase {
  include(context, includeChildrenRecursively) {
    ...
    (_a = this.closingElement) == null ? void 0 : _a.include(context, includeChildrenRecursively);
  }
  render(code, options) {
  ...
  renderAutomaticMode(code, options) {
    ...
    if (selfClosing) {
      code.appendLeft(((_a = keyAttribute == null ? void 0 : keyAttribute.value) == null ? void 0 : _a.end) || end, ")");
    } else {
      closingElement.render(code, options);
    }
  }
  renderAttributes(code, options, factoryName, extractKeyAttribute) {
    ...
    for (const attribute of attributes) {
      if (attribute instanceof JSXAttribute) {
        if (extractKeyAttribute && attribute.name.name === "key") {
          ...
          code.remove(previousEnd, ((_a = attribute.value) == null ? void 0 : _a.start) || attribute.end);
          continue;
        }
        code.appendLeft(previousEnd, ",");
        if (!inObject) {
    ...
    code.remove(((_b = attributes.at(-1)) == null ? void 0 : _b.end) || previousEnd, openingEnd);
...
var JSXFragment = class extends JSXElementBase {
  include(context, includeChildrenRecursively) {
    super.include(context, includeChildrenRecursively);
...
var JSXMemberExpression = class extends NodeBase {
  includeNode(context) {
    this.included = true;
    if (!this.deoptimized)
      this.applyDeoptimizations();
...
var JSXNamespacedName = class extends NodeBase {
};
JSXNamespacedName.prototype.includeNode = onlyIncludeSelf;
var JSXOpeningElement = class extends NodeBase {
...
var JSXOpeningFragment = class extends NodeBase {
  constructor() {
    super(...arguments);
    this.fragment = null;
...
var JSXSpreadChild = class extends NodeBase {
  render(code, options) {
    super.render(code, options);
    const { mode } = this.scope.context.options.jsx;
    if (mode !== "preserve") {
...
var JSXText = class extends NodeBase {
  render(code) {
    const { mode } = this.scope.context.options.jsx;
    if (mode !== "preserve") {
...
var LogicalExpression = class extends NodeBase {
  ...
  include(context, includeChildrenRecursively) {
    this.included = true;
    const usedBranch = this.getUsedBranch();
    if (includeChildrenRecursively || !usedBranch || usedBranch === this.right && this.left.shouldBeIncluded(context)) {
...
var ObjectExpression2 = class extends NodeBase {
  constructor() {
    super(...arguments);
    this.objectEntity = null;
    this.protoProp = null;
  ...
  include(context, includeChildrenRecursively) {
    ...
    (_a = this.protoProp) == null ? void 0 : _a.include(context, includeChildrenRecursively);
  }
  includeNode(context) {
    var _a;
    this.included = true;
    (_a = this.protoProp) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context);
  }
  includePath(path, context) {
    if (!this.included)
      this.includeNode(context);
...
var Property2 = class extends MethodBase {
  ...
  deoptimizeAssignment(destructuredInitPath, init) {
    var _a, _b;
    (_b = (_a = this.value).deoptimizeAssignment) == null ? void 0 : _b.call(_a, this.getPathInProperty(destructuredInitPath), init);
  }
  hasEffects(context) {
  ...
  hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
    var _a, _b;
    return (_b = (_a = this.value).hasEffectsWhenDestructuring) == null ? void 0 : _b.call(_a, context, this.getPathInProperty(destructuredInitPath), init);
  }
  includeDestructuredIfNecessary(context, destructuredInitPath, init) {
    const path = this.getPathInProperty(destructuredInitPath);
    let included = this.value.includeDestructuredIfNecessary(context, path, init) || this.included;
    if (included || (included = this.key.hasEffects(createHasEffectsContext()))) {
...
var PropertyDefinition = class extends NodeBase {
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker) {
    var _a;
    (_a = this.value) == null ? void 0 : _a.deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker);
  }
  deoptimizePath(path) {
    var _a;
    (_a = this.value) == null ? void 0 : _a.deoptimizePath(path);
  }
  getLiteralValueAtPath(path, recursionTracker, origin) {
  ...
  hasEffects(context) {
    var _a;
    return this.key.hasEffects(context) || this.static && !!((_a = this.value) == null ? void 0 : _a.hasEffects(context)) || checkEffectForNodes(this.decorators, context);
  }
  hasEffectsOnInteractionAtPath(path, interaction, context) {
  ...
  includeNode(context) {
    var _a;
    this.included = true;
    (_a = this.value) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context);
    for (const decorator2 of this.decorators) {
      decorator2.includePath(UNKNOWN_PATH, context);
    }
  }
};
...
var ReturnStatement2 = class extends NodeBase {
  ...
  includeNode(context) {
    var _a;
    this.included = true;
    (_a = this.argument) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context);
  }
  initialise() {
    super.initialise();
...
var UnaryExpression = class extends NodeBase {
  ...
  include(context, includeChildrenRecursively, _options) {
    ...
    if (!this.deoptimized)
      this.applyDeoptimizations();
    this.included = true;
    const shouldPreserveArgument = this.argument instanceof Identifier2 && ((_a = this.argument.variable) == null ? void 0 : _a.included);
...
var CHARACTERS_THAT_DO_NOT_REQUIRE_SPACE = /[\s([=%&*+-/<>^|,?:;]/;
...
var YieldExpression = class extends NodeBase {
  applyDeoptimizations() {
    var _a;
    this.deoptimized = true;
    (_a = this.argument) == null ? void 0 : _a.deoptimizePath(UNKNOWN_PATH);
  }
  hasEffects(context) {
    var _a;
    if (!this.deoptimized)
...
function convertProgram(buffer, parent, parentScope) {
  return convertNode2(parent, parentScope, 0, getAstBuffer(buffer));
}
var nodeTypeStrings = [
  "PanicError",
  "ParseError",
...
var bufferParsers = [
  ...
  function arrowFunctionExpression(node, position, buffer) {
    ...
    scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement2);
    node.body = convertNode2(node, scope.bodyScope, buffer[position + 3], buffer);
  },
  function assignmentExpression(node, position, buffer) {
    const { scope } = node;
    node.operator = FIXED_STRINGS[buffer[position]];
    node.left = convertNode2(node, scope, buffer[position + 1], buffer);
    node.right = convertNode2(node, scope, buffer[position + 2], buffer);
  },
  function assignmentPattern(node, position, buffer) {
    const { scope } = node;
    node.left = convertNode2(node, scope, buffer[position], buffer);
    node.right = convertNode2(node, scope, buffer[position + 1], buffer);
  },
  function awaitExpression(node, position, buffer) {
    const { scope } = node;
    node.argument = convertNode2(node, scope, buffer[position], buffer);
  },
  ...
  function breakStatement(node, position, buffer) {
    ...
    node.label = labelPosition === 0 ? null : convertNode2(node, scope, labelPosition, buffer);
  },
  function callExpression(node, position, buffer) {
    ...
    node.callee = convertNode2(node, scope, buffer[position + 2], buffer);
    node.arguments = convertNodeList(node, scope, buffer[position + 3], buffer);
  },
  function catchClause(node, position, buffer) {
    const { scope } = node;
    const parameterPosition = buffer[position];
    const parameter = node.param = parameterPosition === 0 ? null : convertNode2(node, scope, parameterPosition, buffer);
    parameter == null ? void 0 : parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION);
    node.body = convertNode2(node, scope.bodyScope, buffer[position + 1], buffer);
  },
  function chainExpression(node, position, buffer) {
    const { scope } = node;
    node.expression = convertNode2(node, scope, buffer[position], buffer);
  },
  function classBody(node, position, buffer) {
    ...
    if (bodyPosition) {
      ...
      for (let index = 0; index < length; index++) {
        ...
        body[index] = convertNode2(node, buffer[nodePosition] !== 79 && (buffer[nodePosition + 3] & /* the static flag is always first */
        1) === 0 ? scope.instanceScope : scope, nodePosition, buffer);
      }
    } else {
      node.body = [];
    }
  ...
  function classDeclaration(node, position, buffer) {
    ...
    node.id = idPosition === 0 ? null : convertNode2(node, scope.parent, idPosition, buffer);
    const superClassPosition = buffer[position + 2];
    node.superClass = superClassPosition === 0 ? null : convertNode2(node, scope, superClassPosition, buffer);
    node.body = convertNode2(node, scope, buffer[position + 3], buffer);
  },
  function classExpression(node, position, buffer) {
    const { scope } = node;
    node.decorators = convertNodeList(node, scope, buffer[position], buffer);
    const idPosition = buffer[position + 1];
    node.id = idPosition === 0 ? null : convertNode2(node, scope, idPosition, buffer);
    const superClassPosition = buffer[position + 2];
    node.superClass = superClassPosition === 0 ? null : convertNode2(node, scope, superClassPosition, buffer);
    node.body = convertNode2(node, scope, buffer[position + 3], buffer);
  },
  function conditionalExpression(node, position, buffer) {
    const { scope } = node;
    node.test = convertNode2(node, scope, buffer[position], buffer);
    node.consequent = convertNode2(node, scope, buffer[position + 1], buffer);
    node.alternate = convertNode2(node, scope, buffer[position + 2], buffer);
  },
  ...
  function directive(node, position, buffer) {
    ...
    node.expression = convertNode2(node, scope, buffer[position + 1], buffer);
  },
  function doWhileStatement(node, position, buffer) {
    const { scope } = node;
    node.body = convertNode2(node, scope, buffer[position], buffer);
    node.test = convertNode2(node, scope, buffer[position + 1], buffer);
  },
  ...
  function exportAllDeclaration(node, position, buffer) {
    ...
    node.exported = exportedPosition === 0 ? null : convertNode2(node, scope, exportedPosition, buffer);
    node.source = convertNode2(node, scope, buffer[position + 1], buffer);
  ...
  function exportDefaultDeclaration(node, position, buffer) {
    const { scope } = node;
    node.declaration = convertNode2(node, scope, buffer[position], buffer);
  },
  function exportNamedDeclaration(node, position, buffer) {
    ...
    node.source = sourcePosition === 0 ? null : convertNode2(node, scope, sourcePosition, buffer);
    node.attributes = convertNodeList(node, scope, buffer[position + 2], buffer);
    const declarationPosition = buffer[position + 3];
    node.declaration = declarationPosition === 0 ? null : convertNode2(node, scope, declarationPosition, buffer);
  },
  function exportSpecifier(node, position, buffer) {
    const { scope } = node;
    node.local = convertNode2(node, scope, buffer[position], buffer);
    const exportedPosition = buffer[position + 1];
    node.exported = exportedPosition === 0 ? node.local : convertNode2(node, scope, exportedPosition, buffer);
  },
  ...
  function forInStatement(node, position, buffer) {
    ...
    node.left = convertNode2(node, scope, buffer[position], buffer);
    node.right = convertNode2(node, scope, buffer[position + 1], buffer);
    node.body = convertNode2(node, scope, buffer[position + 2], buffer);
  },
  ...
  function forStatement(node, position, buffer) {
    ...
    node.init = initPosition === 0 ? null : convertNode2(node, scope, initPosition, buffer);
    const testPosition = buffer[position + 1];
    node.test = testPosition === 0 ? null : convertNode2(node, scope, testPosition, buffer);
    const updatePosition = buffer[position + 2];
    node.update = updatePosition === 0 ? null : convertNode2(node, scope, updatePosition, buffer);
    node.body = convertNode2(node, scope, buffer[position + 3], buffer);
  },
  function functionDeclaration(node, position, buffer) {
    ...
    scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement2);
    node.body = convertNode2(node, scope.bodyScope, buffer[position + 4], buffer);
  },
  function functionExpression(node, position, buffer) {
    ...
    node.annotationNoSideEffects = annotations.some((comment) => comment.type === "noSideEffects");
    const idPosition = buffer[position + 2];
    node.id = idPosition === 0 ? null : convertNode2(node, node.idScope, idPosition, buffer);
    const parameters = node.params = convertNodeList(node, scope, buffer[position + 3], buffer);
    scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement2);
    node.body = convertNode2(node, scope.bodyScope, buffer[position + 4], buffer);
  },
  ...
  function ifStatement(node, position, buffer) {
    const { scope } = node;
    node.test = convertNode2(node, scope, buffer[position], buffer);
    node.consequent = convertNode2(node, node.consequentScope = new TrackingScope(scope), buffer[position + 1], buffer);
    const alternatePosition = buffer[position + 2];
    node.alternate = alternatePosition === 0 ? null : convertNode2(node, node.alternateScope = new TrackingScope(scope), alternatePosition, buffer);
  },
  function importAttribute(node, position, buffer) {
    const { scope } = node;
    node.key = convertNode2(node, scope, buffer[position], buffer);
    node.value = convertNode2(node, scope, buffer[position + 1], buffer);
  },
  ...
  function importExpression(node, position, buffer) {
    const { scope } = node;
    node.source = convertNode2(node, scope, buffer[position], buffer);
    node.sourceAstNode = convertNode(buffer[position], buffer);
    const optionsPosition = buffer[position + 1];
    node.options = optionsPosition === 0 ? null : convertNode2(node, scope, optionsPosition, buffer);
  },
  ...
  function importSpecifier(node, position, buffer) {
    ...
    node.local = convertNode2(node, scope, buffer[position + 1], buffer);
    node.imported = importedPosition === 0 ? node.local : convertNode2(node, scope, importedPosition, buffer);
  },
  function jsxAttribute(node, position, buffer) {
    const { scope } = node;
    node.name = convertNode2(node, scope, buffer[position], buffer);
    const valuePosition = buffer[position + 1];
    node.value = valuePosition === 0 ? null : convertNode2(node, scope, valuePosition, buffer);
  },
  ...
  function jsxElement(node, position, buffer) {
    const { scope } = node;
    node.openingElement = convertNode2(node, scope, buffer[position], buffer);
    node.children = convertNodeList(node, scope, buffer[position + 1], buffer);
    const closingElementPosition = buffer[position + 2];
    node.closingElement = closingElementPosition === 0 ? null : convertNode2(node, scope, closingElementPosition, buffer);
  },
  ...
  function jsxFragment(node, position, buffer) {
    const { scope } = node;
    node.openingFragment = convertNode2(node, scope, buffer[position], buffer);
    node.children = convertNodeList(node, scope, buffer[position + 1], buffer);
    node.closingFragment = convertNode2(node, scope, buffer[position + 2], buffer);
  },
  ...
  function jsxMemberExpression(node, position, buffer) {
    const { scope } = node;
    node.object = convertNode2(node, scope, buffer[position], buffer);
    node.property = convertNode2(node, scope, buffer[position + 1], buffer);
  },
  function jsxNamespacedName(node, position, buffer) {
    const { scope } = node;
    node.namespace = convertNode2(node, scope, buffer[position], buffer);
    node.name = convertNode2(node, scope, buffer[position + 1], buffer);
  },
  ...
  function labeledStatement(node, position, buffer) {
    const { scope } = node;
    node.label = convertNode2(node, scope, buffer[position], buffer);
    node.body = convertNode2(node, scope, buffer[position + 1], buffer);
  },
  ...
  function literalNumber(node, position, buffer) {
    ...
    node.raw = rawPosition === 0 ? void 0 : buffer.convertString(rawPosition);
  ...
  function memberExpression(node, position, buffer) {
    ...
    node.object = convertNode2(node, scope, buffer[position + 1], buffer);
    node.property = convertNode2(node, scope, buffer[position + 2], buffer);
  },
  function metaProperty(node, position, buffer) {
    const { scope } = node;
    node.meta = convertNode2(node, scope, buffer[position], buffer);
    node.property = convertNode2(node, scope, buffer[position + 1], buffer);
  },
  function methodDefinition(node, position, buffer) {
    ...
    node.key = convertNode2(node, scope, buffer[position + 2], buffer);
    node.value = convertNode2(node, scope, buffer[position + 3], buffer);
  ...
  function newExpression(node, position, buffer) {
    ...
    node.callee = convertNode2(node, scope, buffer[position + 1], buffer);
  ...
  function property(node, position, buffer) {
    ...
    node.value = convertNode2(node, scope, buffer[position + 2], buffer);
    node.kind = FIXED_STRINGS[buffer[position + 3]];
    node.key = keyPosition === 0 ? node.value : convertNode2(node, scope, keyPosition, buffer);
  },
  ...
  function returnStatement(node, position, buffer) {
    ...
    node.argument = argumentPosition === 0 ? null : convertNode2(node, scope, argumentPosition, buffer);
  },
  ...
  function switchStatement(node, position, buffer) {
    const { scope } = node;
    node.discriminant = convertNode2(node, node.parentScope, buffer[position], buffer);
    node.cases = convertNodeList(node, scope, buffer[position + 1], buffer);
  },
  function taggedTemplateExpression(node, position, buffer) {
    const { scope } = node;
    node.tag = convertNode2(node, scope, buffer[position], buffer);
    node.quasi = convertNode2(node, scope, buffer[position + 1], buffer);
  },
  ...
  function tryStatement(node, position, buffer) {
    const { scope } = node;
    node.block = convertNode2(node, scope, buffer[position], buffer);
    const handlerPosition = buffer[position + 1];
    node.handler = handlerPosition === 0 ? null : convertNode2(node, scope, handlerPosition, buffer);
    const finalizerPosition = buffer[position + 2];
    node.finalizer = finalizerPosition === 0 ? null : convertNode2(node, scope, finalizerPosition, buffer);
  },
  function unaryExpression(node, position, buffer) {
    const { scope } = node;
    node.operator = FIXED_STRINGS[buffer[position]];
    node.argument = convertNode2(node, scope, buffer[position + 1], buffer);
  },
  function updateExpression(node, position, buffer) {
    ...
    node.argument = convertNode2(node, scope, buffer[position + 2], buffer);
  },
  ...
  function variableDeclarator(node, position, buffer) {
    const { scope } = node;
    node.id = convertNode2(node, scope, buffer[position], buffer);
...
function convertNode2(parent, parentScope, position, buffer) {
...
function convertNodeList(parent, parentScope, position, buffer) {
  ...
  for (let index = 0; index < length; index++) {
    ...
    list[index] = nodePosition ? convertNode2(parent, parentScope, nodePosition, buffer) : null;
  }
  return list;
}
...
var Module = class _Module {
  ...
  getVariableForExportName(name, { importerForSideEffects, isExportAllSearch, onlyExplicit, searchedNamesAndModules } = EMPTY_OBJECT) {
    ...
    if (reexportDeclaration) {
      ...
      if (!variable) {
        return this.error(logMissingExport(reexportDeclaration.localName, this.id, reexportDeclaration.module.id, !!(options == null ? void 0 : options.missingButExportExists)), reexportDeclaration.start);
      }
  ...
  async setSource({ ast, code, customTransformCache, originalCode, originalSourcemap, resolvedIds, sourcemapChain, transformDependencies, transformFiles, ...moduleOptions }) {
    ...
    if (ast) {
      ...
      const astBuffer = await (0, import_native.parseAsync)(code, false, this.options.jsx !== false);
      timeStart("generate ast", 3);
  ...
  traceVariable(name, { importerForSideEffects, isExportAllSearch, searchedNamesAndModules } = EMPTY_OBJECT) {
    ...
    if (importDescription) {
      ...
      if (!declaration) {
        return this.error(logMissingExport(importDescription.name, this.id, otherModule.id, !!(options == null ? void 0 : options.missingButExportExists)), importDescription.start);
      }
      return declaration;
    }
    return null;
  ...
  getVariableFromNamespaceReexports(name, importerForSideEffects, searchedNamesAndModules) {
    ...
    for (const module of this.exportAllModules) {
      ...
      if (module instanceof ExternalModule || (options == null ? void 0 : options.indirectExternal)) {
...
var copyNameToModulesMap = (searchedNamesAndModules) => searchedNamesAndModules && new Map(Array.from(searchedNamesAndModules, ([name, modules]) => [name, new Set(modules)]));
  ...
  generateFacades() {
    ...
    for (const module of entryModules) {
      if (module.preserveSignature === "allow-extension") {
        ...
        if (canPreserveExports && !module.chunkFileNames.size && module.chunkNames.every(({ isUserDefined }) => !isUserDefined)) {
...
`);
  ...
  getDynamicImportStringAndAttributes(resolution, fileName, node) {
    ...
    if (["es", "cjs"].includes(this.outputOptions.format) && this.outputOptions.externalImportAttributes) {
      ...
      attributes = attributesFromImportAttributes === EMPTY_OBJECT ? true : formatAttributes(attributesFromImportAttributes, this.snippets);
    }
  ...
  getPreserveModulesChunkNameFromModule(module) {
    ...
    if (isAbsolute(idWithoutExtension)) {
      if (preserveModulesRoot && (0, import_node_path.resolve)(idWithoutExtension).startsWith(preserveModulesRoot)) {
      ...
      return this.outputOptions.virtualDirname.replace(/\/$/, "") + "/" + (0, import_node_path.basename)(idWithoutExtension);
    }
  }
    ...
    replaceContext = (pluginContext, plugin) => ({
      ...
      resolve: (source2, importer2, { attributes: attributes2, custom, isEntry: isEntry2, skipSelf } = BLANK) => {
        ...
        if (skipSelf && skip.findIndex((skippedCall) => {
          return skippedCall.plugin === plugin && skippedCall.source === source2 && skippedCall.importer === importer2;
        }) !== -1) {
...
async function resolveId(source, importer, preserveSymlinks, pluginDriver, moduleLoaderResolveId, skip, customOptions, isEntry, attributes, fs2) {
  ...
  if (importer !== void 0 && !isAbsolute(source) && source[0] !== ".")
    return null;
  return addJsExtensionIfNecessary(importer ? (0, import_node_path.resolve)((0, import_node_path.dirname)(importer), source) : (0, import_node_path.resolve)(source), preserveSymlinks, fs2);
}
async function addJsExtensionIfNecessary(file, preserveSymlinks, fs2) {
  return await findFile(file, preserveSymlinks, fs2) ?? await findFile(file + ".mjs", preserveSymlinks, fs2) ?? await findFile(file + ".js", preserveSymlinks, fs2);
}
async function findFile(file, preserveSymlinks, fs2) {
  try {
    const stats = await fs2.lstat(file);
    if (!preserveSymlinks && stats.isSymbolicLink())
      return await findFile(await fs2.realpath(file), preserveSymlinks, fs2);
    if (preserveSymlinks && stats.isSymbolicLink() || stats.isFile()) {
      const name = (0, import_node_path.basename)(file);
      const files = await fs2.readdir((0, import_node_path.dirname)(file));
...
function getMatcherString(glob, cwd) {
  ...
  const resolved = (0, import_node_path.resolve)(cwd, glob);
...
function createFilter2(exclude, include) {
  ...
  return (input) => {
    if (exclude == null ? void 0 : exclude.some((filter) => filter(input))) {
      return false;
    }
    if (include == null ? void 0 : include.some((filter) => filter(input))) {
      return true;
    }
...
function normalizeFilter(filter) {
  ...
  return {
    exclude: filter.exclude ? ensureArray(filter.exclude) : void 0,
    include: filter.include ? ensureArray(filter.include) : void 0
  };
}
    ...
    for (const module of this.modules) {
      for (const importDescription of module.importDescriptions.values()) {
        if (importDescription.name !== "*") {
          ...
          if (!variable) {
            module.log(LOGLEVEL_WARN, logMissingExport(importDescription.name, module.id, importDescription.module.id, !!(options == null ? void 0 : options.missingButExportExists)), importDescription.start);
          }
        }
      }
    }
...
async function writeOutputFile(outputFile, outputOptions, { fs: { mkdir, writeFile } }) {
  ...
  await mkdir((0, import_node_path.dirname)(fileName), { recursive: true });
...
function requirePicocolors() {
  ...
  let p = process || {}, argv = p.argv || [], env2 = p.env || {};
  let isColorSupported = !(!!env2.NO_COLOR || argv.includes("--no-color")) && (!!env2.FORCE_COLOR || argv.includes("--color") || p.platform === "win32" || (p.stdout || {}).isTTY && env2.TERM !== "dumb" || !!env2.CI);
...
var picocolorsExports = requirePicocolors();
var pc = getDefaultExportFromCjs(picocolorsExports);
var { bold, cyan, dim, red } = pc.createColors(import_node_process.env.FORCE_COLOR !== "0" && !import_node_process.env.NO_COLOR);
...
function checkWatchConfig(config) {
  ...
  for (const item of config) {
    if (typeof item.watch !== "boolean" && ((_a = item.watch) == null ? void 0 : _a.allowInputInsideOutputPath)) {
      break;
    }
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-ZYGNHTIA.js");
...
rollup/dist/es/shared/node-entry.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-QRKYWRSN.js.map
